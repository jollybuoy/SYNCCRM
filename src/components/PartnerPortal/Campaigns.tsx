...
import { Plus, Target, Users, Calendar, TrendingUp, Filter, X } from 'lucide-react';
                  ...
                  <div key={goal.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-sm text-gray-600 mt-1">
                        {goal.target ? Math.round((goal.achieved / goal.target) * 100) : 0}%
                      </p>
                    </div>
                  </div>
                ))}
...
export default function Campaigns() {
  ...
  function mapDbToCampaign(row: any): Campaign {
    return {
      ...
      metrics: row.metrics ?? {
        customersTargeted: 0, customersContacted: 0,
        meetingsScheduled: 0, opportunitiesCreated: 0, revenue: 0
      },
      goals: row.goals ?? [],
      targetCustomers: (row.target_customer_ids ?? []) as number[],
      targetManufacturers: [] // (we’ll read via join later if needed for UI)
    };
  }
  ...
  async function handleCreateCampaign(e: React.FormEvent) {
    ...
    // 2) Link manufacturers (join table)
    if (selectedManufacturerIds.length > 0) {
      const linkRows = selectedManufacturerIds.map((mid) => ({
        campaign_id: newCampaignId,
        manufacturer_id: mid
      }));
      const { error: linkErr } = await supabase.from('campaign_manufacturers').insert(linkRows);
      if (linkErr) {
        // Not fatal to campaign creation, but tell the user.
        alert(`Campaign created, but linking manufacturers failed: ${linkErr.message}`);
      }
    }
    ...
    setBudget(''); setDescription(''); setSelectedManufacturerIds([]);
          ...
          <div className="flex flex-wrap gap-4">
            <div>
              ...
              <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}
                ...
                <option value="paused">Paused</option><option value="completed">Completed</option><option value="draft">Draft</option>
              </select>
            </div>
            ...
            <div>
              ...
              <select value={filterType} onChange={(e) => setFilterType(e.target.value)}
                ...
                <option value="government">Government</option><option value="location">Location</option><option value="industry">Industry</option>
              </select>
            </div>
        ...
        <form className="space-y-4" onSubmit={handleCreateCampaign}>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <option value="university">University</option><option value="hospital">Hospital</option>
                <option value="government">Government</option><option value="location">Location-based</option>
          ...
          <div className="flex justify-end space-x-3 pt-4">
            ...
            <button type="button" className="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-300"
                    disabled={saving}>Save as Draft</button>
            <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-60"
                    disabled={saving}>{saving ? 'Creating…' : 'Create Campaign'}</button>
          </div>
        </form>
      </Modal>
    </>
